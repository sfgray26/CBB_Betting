...
- Creating a new task contract in `.claude/delegation/`.
...
When starting a new delegation, create a JSON bundle in `.claude/delegation/YYYYMMDD-task-name.json` following this schema:
...
2. Update `.claude/delegation/task.json` status to `COMPLETED`.
...
- **No Direct Merges:** Sub-agents (except Claude) are forbidden from merging their own branches to `main`.
...
-> *Action:* Create `.claude/delegation/20260427-mlb-hydration.json` and update `HANDOFF.md`.
