...
class OpenClawLite:
    ...
    HIGH_RISK_KEYWORDS = [
        ...
        "lineup", "starting", "star", "key player"
    ]
    
    CONFLICT_KEYWORDS = [
        "conflicting", "disagree", "contradictory", "unclear",
        "rumor", "speculation", "reportedly"
    ]
    ...
    def check_integrity_heuristic(
        ...
        # Decision logic
        if high_risk_hits >= 3 or "star" in text_lower and "out" in text_lower:
            return IntegrityResult(
                verdict="CAUTION",
                confidence=0.7,
                reasoning=f"Multiple risk keywords detected ({high_risk_hits} hits)",
                source="heuristic"
            )
        ...
        if "out" in text_lower and ("star" in text_lower or "starter" in text_lower):
            return IntegrityResult(
                verdict="CAUTION",
                confidence=0.75,
                reasoning="Key player absence mentioned",
                source="heuristic"
            )
        
        if recommended_units >= 1.5:
            # High stakes — flag for review even if clean
            return IntegrityResult(
                verdict="CONFIRMED",
                confidence=0.8,
                reasoning="No red flags in search, but high stakes warrant caution",
                source="heuristic"
            )
