...
def test_channel_config():
    ...
    token = _bot_token()
    if token:
        print(f"✅ DISCORD_BOT_TOKEN: Configured (len={len(token)})")
    else:
        print("❌ DISCORD_BOT_TOKEN: NOT SET — All notifications will be skipped")
        return False
...
def send_test_messages(dry_run=False):
    ...
    if dry_run:
        print("⚠️  DRY RUN MODE — No actual messages sent")
        print()
    ...
    # Test CBB channels
    test_cases = [
        ("cbb-bets", "🏀 Test bet notification", {"color": 0x2ECC71}),
        ("cbb-morning-brief", "📋 Test morning brief", {"color": 0x3498DB}),
        ("cbb-alerts", "⚠️ Test line movement alert", {"color": 0xF1C40F}),
        ("cbb-tournament", "🏆 Test tournament update", {"color": 0x9B59B6}),
        
        ("fantasy-lineups", "⚾ Test lineup recommendation", {"color": 0x3498DB}),
        ("fantasy-waivers", "🎯 Test waiver suggestion", {"color": 0x2ECC71}),
        ...
        ("openclaw-escalations", "🚨 Test high-stakes escalation", {"color": 0xE74C3C}),
        ("openclaw-health", "🛰️ Test health report", {"color": 0x2ECC71}),
        
        ("system-errors", "❌ Test system error", {"color": 0xE74C3C}),
        ("system-logs", "📝 Test routine log entry"),
        ("data-alerts", "⚠️ Test data degradation", {"color": 0xE67E22}),
        
        ("general", "💬 Test general message"),
    ]
...
def main():
    ...
    if failed_count > 0:
        ...
        print("⚠️  Dry run complete — no messages actually sent")
        sys.exit(0)
    else:
        print("⚠️  No channels configured or all skipped")
        sys.exit(1)
