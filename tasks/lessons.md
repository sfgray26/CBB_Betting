...
### Whole-file regeneration hallucinates schema details — graft edits surgically
- **Lesson**: Never regenerate an entire module to add a small feature. Commit `c780d56` rewrote all of `backend/services/injury_overlay.py` for a ~30-line expired-ETA feature and hallucinated `PlayerIDMapping.yahoo_player_key`/`bdl_player_id` (real columns: `yahoo_key`/`bdl_id`), used an undefined `logger`, inverted injury penalty math (subtractive → multiplicative, boosting 60-Day IL ×1.25), and silently dropped the yahoo_id fallback resolver. Always verify ORM attribute names against `backend/models.py` before writing queries, and keep diffs scoped to the feature.
- **Context**: June 12 deploy review blocked production; fixed in `4a1c8bd` by restoring pre-commit code and grafting only the expired-ETA display (real-time date comparison, no mid-request `db.commit()` — the nightly `check_expired_eta` watchdog already persists the flag).
