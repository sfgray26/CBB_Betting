...
from .master_stat_registry import (
    ALL_STATS,
    BATTING_STATS,
    DISPLAY_STATS,
    PITCHING_STATS,
...
def build_contract(
    *,
    ...
    supporting_section: dict[str, StatEntry] = {}
    ...
    for code in sorted(referenced):
        ...
        supporting_section[code] = _to_entry(
            SUPPORTING_STATS[code], is_scoring_category=False
        )
...
def _collect_referenced_supporting_stats(
    ...
    while frontier:
        code = frontier.pop()
        agg = SUPPORTING_STATS[code].aggregation
        for candidate in (agg.numerator_stat, agg.denominator_stat, agg.source_stat):
