...
College Basketball (CBB) Edge Analyzer — a production betting framework for NCAA Division 1 basketball. The system fetches real-time odds, integrates multi-source ratings (KenPom, BartTorvik, EvanMiya), runs a Version 7 Monte Carlo betting model, and tracks performance via Closing Line Value (CLV). It is **conservative by design**: the model PASSes on 85-95% of games and only recommends bets when the lower-bound edge is positive after uncertainty adjustments.
...
├── tests/
│   ├── test_betting_model.py # Monte Carlo CI, Kelly, edge calculations
...
pytest tests/test_betting_model.py::TestMonteCarloCI -v
pytest tests/test_betting_model.py::TestMonteCarloCI::test_ci_bounds_are_valid -v
```

Tests require a PostgreSQL database. The CI workflow uses a PostgreSQL 15 service container. Locally, ensure `DATABASE_URL` in `.env` points to a running PostgreSQL instance.
...
- **Penalty budget with ceiling**: Penalties applied for data staleness, low sample sizes, etc.
- **Vig removal**: Proper conversion from American odds to true probabilities
...
|----------|-----|----------|
| Daily at 3 AM ET | Nightly analysis | `run_nightly_analysis()` — fetch odds, ratings, run model, persist predictions, create paper bets |
...
- `GET /api/performance` — performance summary stats
...
- **Dataclasses** for internal data transfer objects (e.g., `GameAnalysis` in `betting_model.py`)
- **Type hints** on function signatures throughout the codebase

### Naming

- Files: `snake_case.py`
- Classes: `PascalCase` (e.g., `CBBEdgeModel`, `GameAnalysis`, `OddsAPIClient`)
- Functions/variables: `snake_case`
- Constants: `UPPER_SNAKE_CASE` (e.g., `BASE_URL`, `API_KEY_HEADER`)
...
- Test structure: class-based grouping by feature (`TestMonteCarloCI`, `TestKellyFraction`)
